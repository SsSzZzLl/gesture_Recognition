...
from backend.data.dataset_no_aug import GestureDatasetNoAug, train_transform_no_aug, val_transform
from backend.models.attention_cnn import AttentionCNN
...
def main():
    ...
    save_root = os.path.join(backend_dir, "experiments", "results", "attention_cnn_optimized_v2")
    ...
    # 加载数据集
    train_dataset = GestureDatasetNoAug(
        data_root,
        split="train",
        transform=train_transform_no_aug
    )
    val_dataset = GestureDatasetNoAug(
        data_root,
        split="val",
    ...
    plot_confusion_matrix(
        ...
        save_path=os.path.join(save_root, "注意力CNN_混淆矩阵.png")
    )

    print(f"注意力CNN实验完成，结果保存至: {save_root}")
